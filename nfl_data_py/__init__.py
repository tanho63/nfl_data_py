...
def import_pbp_data(
    ...
    url1 = (
        r"https://github.com/nflverse/nflverse-data/releases/download/pbp/play_by_play_"
    )
    url2 = r".parquet"
    appname = "nfl_data_py"
        ...
        for year in years:
            ...
            try:
                data = pandas.read_parquet(
                    path, columns=columns if columns else None, engine="auto"
                )
                ...
                if include_participation and not cache:
                    path = r"https://github.com/nflverse/nflverse-data/releases/download/pbp_participation/pbp_participation_{}.parquet".format(
                        year
                    )
...
def cache_pbp(years, downcast=True, alt_path=None):
    ...
    for year in years:
        ...
        try:
            ...
            if year >= 2016:
                path2 = r"https://github.com/nflverse/nflverse-data/releases/download/pbp_participation/pbp_participation_{}.parquet".format(
                    year
                )
...
def import_weekly_data(years, columns=None, downcast=True, thread_requests=False):
    ...
    if thread_requests:
        ...
        # read weekly data
        data = pandas.concat(
            [pandas.read_parquet(url.format(x), engine="auto") for x in years]
        )
...
def import_seasonal_data(years, s_type="REG"):
    ...
    all_stats = (
        ...
        .merge(pgstats, how="left", on=["recent_team", "season", "week"])
        .fillna(0)
    )
    season_stats = (
        all_stats.drop(["recent_team", "week"], axis=1)
    ...
    season_stats["yac_sh"] = (
        season_stats["receiving_yards_after_catch"] / season_stats["p_yac"]
    )
    ...
    season_stats["rfd_sh"] = (
        season_stats["receiving_first_downs"] / season_stats["p_fds"]
    )
    ...
    season_stats["ppr_sh"] = (
        season_stats["fantasy_points_ppr"] / season_stats["ppr_pts"]
    )

    data.drop(["recent_team", "week"], axis=1, inplace=True)
    szn = (
        data.groupby(["player_id", "season", "season_type"])
        .sum(numeric_only=True)
        .reset_index()
        .merge(
            data[["player_id", "season", "season_type"]]
            .groupby(["player_id", "season"])
            .count()
            .reset_index()
            .rename(columns={"season_type": "games"}),
            how="left",
            on=["player_id", "season"],
        )
    )
...
def see_pbp_cols():
    ...
    data = pandas.read_parquet(
        r"https://github.com/nflverse/nflverse-data/releases/download/pbp/play_by_play_2020.parquet",
        engine="auto",
    )
    cols = data.columns
...
def see_weekly_cols():
    ...
    data = pandas.read_parquet(
        r"https://github.com/nflverse/nflverse-data/releases/download/player_stats/player_stats_2020.parquet",
        engine="auto",
    )
    cols = data.columns
...
def __import_rosters(release, years, columns=None):
    ...
    if columns is not None and not (
        ...
        and all(isinstance(x, str) for x in columns)
        and len(columns) > 0
    ):
    ...
    rosters = pandas.concat(
        [pandas.read_parquet(uri.format(y)) for y in years], ignore_index=True
    )

    # Post-import processing
    rosters["birth_date"] = pandas.to_datetime(rosters.birth_date)
    rosters.rename(
        columns={"gsis_id": "player_id", "full_name": "player_name"}, inplace=True
    )
    if columns:
...
def import_weekly_rosters(years, columns=None):
    ...
    roster_dates = pandas.to_datetime(
        rosters.merge(
            week_team_dates, on=["season", "week", "team"], how="left"
        ).gameday
    )
...
def import_seasonal_rosters(years, columns=None):
    ...
    if "birth_date" in rosters.columns:
        ...
        rosters["age"] = (
            rosters.szn_start.dt.year
            - rosters.birth_date.dt.year
            + numpy.where(
...
def import_players():
    ...
    df = pandas.read_parquet(
        r"https://github.com/nflverse/nflverse-data/releases/download/players/players.parquet"
    )
    return df
...
def import_team_desc():
    ...
    # import desc data
    df = pandas.read_csv(
        r"https://github.com/nflverse/nflfastR-data/raw/master/teams_colors_logos.csv"
    )
...
def import_officials(years=None):
    ...
    df = pandas.read_csv(
        r"https://raw.githubusercontent.com/nflverse/nfldata/master/data/officials.csv"
    )
...
def import_sc_lines(years=None):
    ...
    df = pandas.read_csv(
        r"https://raw.githubusercontent.com/nflverse/nfldata/master/data/sc_lines.csv"
    )
...
def import_draft_picks(years=None):
    ...
    df = pandas.read_parquet(
        r"https://github.com/nflverse/nflverse-data/releases/download/draft_picks/draft_picks.parquet",
        engine="auto",
    )
...
def import_draft_values(picks=None):
    ...
    df = pandas.read_csv(
        r"https://raw.githubusercontent.com/nflverse/nfldata/master/data/draft_values.csv"
    )
...
def import_combine_data(years=None, positions=None):
    ...
    df = pandas.read_parquet(
        r"https://github.com/nflverse/nflverse-data/releases/download/combine/combine.parquet",
        engine="auto",
    )
...
def import_contracts():
    ...
    df = pandas.read_parquet(
        r"https://github.com/nflverse/nflverse-data/releases/download/contracts/historical_contracts.parquet"
    )
...
def import_ngs_data(stat_type, years=None):
    ...
    url = r"https://github.com/nflverse/nflverse-data/releases/download/nextgen_stats/ngs_{0}.parquet".format(
        stat_type
    )
...
def import_qbr(years=None, level="nfl", frequency="season"):
    ...
    url = r"https://raw.githubusercontent.com/nflverse/espnscrapeR-data/master/data/qbr-{}-{}.csv".format(
        level, frequency
    )
...
def import_weekly_pfr(s_type, years=None):
    ...
    df = pandas.concat([pandas.read_parquet(url.format(s_type, yr)) for yr in years])

    return df[df.season.isin(years)] if years else df


def import_snap_counts(years) -> pandas.DataFrame:
...
def import_ftn_data(years, columns=None, downcast=True, thread_requests=False):
    ...
    if thread_requests:
        ...
        data = pandas.concat(
            [
                pandas.read_parquet(url.format(x), engine="auto", columns=columns)
                for x in years
            ]
        )
